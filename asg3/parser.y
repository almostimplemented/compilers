...
structdef   : structhead '}'                { free_ast($2); $$ = $1; }
            ;
structhead  : structhead fielddecl ';'      { free_ast($3); 
                                              $$ = adopt1($1, $2); }
...
ifelse      : TOK_IF '(' expr ')' statement TOK_ELSE statement  { free_ast($2);
                                                                  ...
                                                                  $$ = adopt3sym($1,
                                                                        $3, $5, $7,
                                                                        TOK_IFELSE); }
            | TOK_IF '(' expr ')' statement %prec "then"        { free_ast($2);
                                                                  free_ast($4);
                                                                  $$ = adopt2($1,
                                                                        $3, $5); }
            ;
...
allocator   : TOK_NEW TOK_IDENT '(' ')'         { free_ast($3); free_ast($4);
                                                  $2->symbol = TOK_TYPEID; 
                                                  $$ = adopt1($1, $2); }
            | TOK_NEW TOK_STRING  '(' expr ')'  { free_ast($2); free_ast($3); free_ast($5);
...
call        : TOK_IDENT '(' ')'         { free_ast($3);
            ...
            | args ')'                  { free_ast($2);
                                          $$ = $1; }
            ;
args        : TOK_IDENT '(' expr        { $$ = adopt2sym($2, $1, $3, TOK_CALL); }
