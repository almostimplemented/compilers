...
//
// DESCRIPTION
//    Part four of the ongoing compiler project. Generates
//    <prog>.ast, <prog>.tok, <prog>.str, and <prog>.sym.
...
//    The fourth file shows the symbol table.
//
...
int main (int argc, char** argv) {
    ...
    string oilfilename = filename + ".oil";
    oilfile = fopen(oilfilename.c_str(), "w");

    // parse
    yyparse();
    yyin_cpp_pclose();

    int traverse_rc = traverse_astree(yyparse_astree);

    // generate .oil file
    if (traverse_rc == 0)
        emit_code(yyparse_astree);
